...
def initialize_csv():
    with open(csv_filename, mode='w', newline='') as file:
        ...
        writer.writerow(["Timestamp", "Tag Status", "Tag ID", "Speed"])

def speed_callback(speed):
    timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    tag_status = rfid_reader.tag_status
    tag_id = "RFID IS READING"
    data = f"Speed: {speed} km/h"
    print(data)
    rfid_reader.update_speed(speed)  # Update the current speed in RFID reader
    log_speed_data(timestamp, tag_status, tag_id, speed)
...
def send_to_kalibrasi(tag_name, tag_id, timestamp, speed):
    ...
    with open(csv_filename, mode='a', newline='') as file:
        writer = csv.writer(file)
        writer.writerow([timestamp, tag_name, tag_id, speed])
    print(f"Logged to {csv_filename}: Tag {tag_name}, ID {tag_id}, Timestamp {timestamp}, Speed {speed}")

def log_speed_data(timestamp, tag_status, tag_id, speed):
    # Log the speed data to the CSV file
    with open(csv_filename, mode='a', newline='') as file:
        writer = csv.writer(file)
        writer.writerow([timestamp, tag_status, tag_id, speed])
    print(f"Logged to {csv_filename}: Timestamp {timestamp}, Tag Status {tag_status}, Tag ID {tag_id}, Speed {speed}")
